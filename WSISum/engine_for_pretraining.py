...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
    ...
    for step, (batch, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        images1, bool_masked_pos = batch
        images = images1[:, :, :-1, :]
        WSIfeatures = images1[:, :, -1, :].squeeze(1)
        images = images.to(device, non_blocking=True)
        WSIfeatures = WSIfeatures.to(device, non_blocking=True)
        ...
        with torch.no_grad():
            ...
            images_patch = images
            images_patch = images_patch.squeeze(1)
            B, _, C = images_patch.shape
            labels = images_patch[bool_masked_pos[:,1:]].reshape(B, -1, C)

        with torch.amp.autocast('cuda'):
            outputs ,cls = model(images, bool_masked_pos)
            # loss = loss_func(input=outputs, target=labels)
            loss = loss_func(input=outputs, target=labels) +loss_func(input=cls,target=WSIfeatures)
