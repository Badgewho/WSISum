...
class PretrainVisionTransformerDecoder(nn.Module):
    ...
    def forward(self, x, return_token_num):
        ...
        if return_token_num > 0:
            x1 = self.head(self.norm(x[:, -return_token_num:])) # only return the mask tokens predict pixels
            x_cls = self.head(self.norm(x[:,0])) 
        else:
            x1 = self.head(self.norm(x)) # [B, N, 3*16^2]
            x_cls = self.head(self.norm(x[:,0])) # [B, N, 3*16^2]
        return x1,x_cls
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        x , x_cls= self.decoder(x_full, pos_emd_mask.shape[1]) # [B, N_mask, 3 * 16 * 16]
